...
const schema = z.object({
  nomorPegawai: z.string().min(1).max(50),
  nama: z.string().min(1).max(100),
  jabatanId: z.preprocess((value) => {
    if (typeof value === 'string' && value.trim() === '') return null
    return value
  }, z.coerce.number().nullable().optional()),
  jenisKelamin: z.enum(['L', 'P']),
  email: z.preprocess(emptyStringToNull, z.string().email().nullable().optional()),
  noHandphone: z.preprocess(emptyStringToNull, z.string().max(20).nullable().optional()),
  alamat: z.preprocess(emptyStringToNull, z.string().nullable().optional()),
  username: z.string().min(1).max(50),
  password: z.string().min(6),
  statusPegawai: z.enum(['aktif', 'nonaktif']).default('aktif'),
})
